            ...
            if ($autoUpdate) {
                try {
                    ...
                    if ($lastCheckedBuildID -gt $localBuildID) {
                        ...
                        $serverUpdate = Start-Process -FilePath "$steamCMD\steamcmd.exe" -ArgumentList "+@ShutdownOnFailedCommand 1 +@NoPromptForPassword 1 +login anonymous +app_update 2394010 validate +quit" -PassThru
                        $serverUpdate.WaitForExit()
                    } else {
